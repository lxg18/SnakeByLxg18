...
let textScoreSizeCoef = 1.5; // для правильного отображения надписи
...
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|BB|PlayBook|IEMobile|Windows Phone|Kindle|Silk|Opera Mini/i.test(navigator.userAgent)) {
  ...
  alert("\n------------------Цвет еды------------------- \nСиняя - телепорт \nЗелёная - съедобный хвост \nКрасная - снимает эффекты")
} 
else {
  alert("\n----------------Управление------------------ \nСтрелки - направление движения \nПробел - Пауза \nCtrl - показать/скрыть панель с кнопками \n------------------Цвет еды------------------- \nСиняя - телепорт \nЗелёная - съедобный хвост \nКрасная - снимает эффекты")
...
function createBlocks() {
  ...
  text.style.lineHeight = textScoreSizeCoef*100+"%";
  text.style.fontSize = (blockCountSqrt * size / textScoreSizeCoef) + "px";
...
function renderBlocks() {
  ...
  // изменение коэф. размера надписи в зависимости от разрядов числа очков 
  let tempScore = score;
  textScoreSizeCoef = 1.5;
  while (1) {
    if (tempScore < 1000) break;
    else {      
      tempScore /= 10;
      textScoreSizeCoef += 0.5;
    }
  }
  text.style.lineHeight = textScoreSizeCoef*100+"%";
  text.style.fontSize = (blockCountSqrt * size / textScoreSizeCoef) + "px";
